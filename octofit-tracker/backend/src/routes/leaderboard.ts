...
router.post('/', async (req, res, next) => {
  try { res.status(201).json(await Leaderboard.create(req.body)); } catch (error) { next(error); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    if (!await Leaderboard.findByIdAndDelete(req.params.id)) { res.status(404).json({ error: 'Leaderboard entry not found' }); return; }
    res.status(204).send();
  } catch (error) { next(error); }
});
