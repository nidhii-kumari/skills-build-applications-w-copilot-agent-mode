...
router.post('/', async (req, res, next) => {
  try { res.status(201).json(await Workout.create(req.body)); } catch (error) { next(error); }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    if (!await Workout.findByIdAndDelete(req.params.id)) { res.status(404).json({ error: 'Workout not found' }); return; }
    res.status(204).send();
  } catch (error) { next(error); }
});
